...
export const DestinationCard: React.FC<Props> = ({ destination }) => {
  ...
  const shareDestination = async () => {
    const url = window.location.origin + '/destinations/' + encodeURIComponent(destination.id);

    try {
      if (navigator.share) {
        await navigator.share({
          title: destination.name + ', ' + destination.state,
          text: 'Explore ' + destination.name + ' in ' + destination.state + ' with IndianTravel.',
          url,
        });
        return;
      }
  ...
  return (
    ...
    >
      <Link href={'/destinations/' + destination.id} className="block flex-1 group">
        <div className="relative h-48 w-full overflow-hidden bg-gray-100">
          {!isLoaded && <div className="absolute inset-0 z-10 animate-pulse bg-gray-200" />}
          <Image
            src={imageUrl}
          ...
          <div className="absolute top-3 left-3 flex gap-2">
            ...
            <span className={'px-2 py-1 text-xs font-bold rounded-full backdrop-blur-md text-white shadow-lg ' + (destination.popularity === 'Hidden' ? 'bg-purple-600/80 shadow-purple-500/50' : 'bg-blue-600/80 shadow-blue-500/50')}>
        ...
        <div className="p-5 flex-1 flex flex-col">
          ...
          <div className="mt-4 pt-4 border-t border-gray-100 flex items-center justify-between text-xs font-medium text-gray-500">
            ...
            <div className="flex gap-1">
              {destination.type.slice(0, 2).map((type, index) => (
                <span key={index} className="bg-gray-100 px-2 py-1 rounded capitalize">{type}</span>
              ))}
            </div>
          </div>
        </div>
      ...
      <div className="flex items-center justify-end px-5 pb-4">
        <button
          ...
          className="inline-flex min-h-9 items-center gap-2 rounded-lg px-3 text-xs font-bold text-blue-700 transition-colors hover:bg-blue-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
        >
